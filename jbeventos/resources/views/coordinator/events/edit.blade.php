            ...
            <div class="bg-white shadow-md rounded p-6">
                ...
                <form action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        {{-- Controle para remoção da imagem de capa --}}
                        <input type="hidden" name="remove_event_image" id="remove_event_image" value="0">

                        {{-- Preview da imagem de capa atual e adicionada --}}
                        <div id="event_image_preview" class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
                            @if ($event->event_image)
                                <div
                                    ...
                                    {{-- Botão para remover imagem de capa --}}
                                    <button type="button"
                    ...
                    {{-- Input para múltiplas imagens extras --}}
                    <div>
                        ...
                        {{-- Preview das imagens extras atuais --}}
                        <div id="event_images_preview" class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
                            @foreach ($event->images as $image)
                                <div
                                    data-filename="{{ basename($image->image_path) }}"
                                    class="relative rounded overflow-hidden flex items-center justify-center bg-gray-100 w-full max-w-full aspect-[2/1]">
                                    <img src="{{ asset('storage/' . $image->image_path) }}" alt="Imagem atual"
                                        class="object-contain w-full h-full">
                    
                                    {{-- Botão para remover imagem extra --}}
                                    <button type="button"
                                        class="absolute z-20 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-700"
                                        onclick="document.getElementById('remove_event_image_{{ $image->id }}').value = '1'; this.parentElement.style.display = 'none';">
                                        ×
                                    </button>
                                </div>
                    
                                {{-- Input hidden para sinalizar remoção da imagem extra --}}
                    ...
                    {{-- Datas: início e exclusão automática --}}
                    ...
                    {{-- Coordenador responsável (exibido como leitura, id enviado via hidden) --}}
                    <div class="mb-4">
                    ...
                    {{-- Tipo do evento (exibido e enviado como hidden) --}}
                    <div class="mb-4">
                    ...
                    {{-- Curso do evento (se for tipo curso) --}}
                    ...
                    {{-- Botões para enviar formulário ou cancelar --}}

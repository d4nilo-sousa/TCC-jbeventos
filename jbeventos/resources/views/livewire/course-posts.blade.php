<div>
    ...
    @if ($isCoordinator)
        <div class="mb-5 p-4 bg-gray-50 rounded-xl border border-gray-200 shadow-sm">
            ...
            <form wire:submit.prevent="createPost">
                ...
                <div class="flex items-center justify-between mt-2">
                    ...
                    <label for="file-upload" class="bg-blue-100 text-blue-600 px-3 py-1.5 rounded-full cursor-pointer hover:bg-blue-200 transition text-sm font-semibold flex items-center gap-1.5">
                    ...
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-1.5 px-5 rounded-lg shadow-md transition text-sm">
                        Postar
                    </button>
                </div>
    ...
    @if (session()->has('success'))
        <div class="mb-3 p-3 bg-green-100 text-green-700 rounded-lg text-sm">{{ session('success') }}</div>
    @endif
    @if (session()->has('error'))
        <div class="mb-3 p-3 bg-red-100 text-red-700 rounded-lg text-sm">{{ session('error') }}</div>
    @endif
    ...
    <div class="space-y-4">
        @forelse ($posts as $post)
            {{-- Card de Post --}}
            <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:border-blue-300 transition-colors duration-200">
                <div class="flex items-start gap-3 mb-3">
                    <img src="{{ $post->author->user_icon ? asset('storage/' . $post->author->user_icon) : asset('images/default-icon.png') }}"
                            class="w-9 h-9 rounded-full object-cover">
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            <a href="#" class="text-stone-800 font-semibold text-sm hover:underline">{{ $post->author->name }}</a>
                ...
                <p class="text-gray-700 text-sm mb-3 whitespace-pre-line">{{ $post->content }}</p>

                @if(!empty($post->images))
                    <div class="mb-3">
                        @if(count($post->images) === 1)
                            <div class="rounded-lg overflow-hidden border border-gray-200">
                                <img src="{{ asset('storage/' . $post->images[0]) }}" alt="Imagem do post" class="object-cover w-full h-auto max-h-64">
                            </div>
                        @else
                ...
                <div class="border-t pt-3">
                    <h5 class="text-xs font-semibold mb-2 text-gray-600">Respostas ({{ $post->replies->count() }})</h5>
                    <div class="space-y-3">
                        @foreach ($post->replies as $reply)
                            <div class="flex items-start gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center justify-between">
                                        <p class="text-xs font-medium text-gray-800">{{ $reply->author->name }}
                                            <span class="text-gray-500 ml-1 font-normal">{{ $reply->created_at->diffForHumans() }}</span>
                                        </p>
                                    ...
                                    <p class="text-xs text-gray-700 mt-1 whitespace-pre-line">{{ $reply->content }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>

                    <form wire:submit.prevent="createReply({{ $post->id }})" class="mt-3">
                        <textarea wire:model.defer="newReplyContent.{{ $post->id }}" rows="1"
                            class="w-full border-gray-300 rounded-lg shadow-sm text-sm focus:border-blue-500 focus:ring-blue-500 resize-none"
        ...
        @empty
            <div class="text-center p-8 bg-gray-50 rounded-lg shadow-sm">
                <p class="text-gray-500 text-sm">Nenhum post foi criado neste curso ainda. O coordenador pode criar o primeiro!</p>
            </div>
        @endforelse
    </div>

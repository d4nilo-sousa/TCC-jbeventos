<div class="space-y-4" x-data="{
    ...
    <div class="bg-white p-6 rounded-xl border shadow-lg sticky top-0 z-10 transition-shadow hover:shadow-xl" id="main-comment-form">
    ...
    <div class="flex items-center gap-3 mt-4">
        ...
        <label for="media-upload" class="cursor-pointer">
            <div class="flex items-center gap-2 text-sm text-gray-600 px-4 py-2 border border-gray-300 rounded-full hover:bg-gray-100 transition-colors shadow-sm">
        ...
        @if ($media)
            ...
            <button wire:click="$set('media', null)" class="text-red-400 hover:text-red-600 text-xs transition-colors" title="Remover arquivo">
        ...
        <button wire:click="addComment"
            class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded-lg transition-colors shadow-lg disabled:bg-blue-300"
    ...
    <div class="p-5 bg-white shadow-md rounded-xl border transition-all duration-300 hover:shadow-lg" id="comment-{{ $comment->id }}">
            ...
            <div class="mt-0" id="edit-form-{{ $comment->id }}">
                <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-300 shadow-inner">
                    <div class="mb-3 text-sm text-gray-600 flex items-center justify-between">
                        ...
                        <button wire:click="cancelEdit" class="text-xs text-gray-500 hover:text-gray-800 hover:underline flex items-center font-medium transition-colors">
        ...
        @if (!$isEditing)
            <div class="flex items-start gap-4">
                ...
                <a href="{{ route($profileRoute, $comment->user->id) }}">
                    <img src="{{ $comment->user->user_icon_url }}" alt="{{ $comment->user->name }}" class="w-10 h-10 rounded-full shadow-sm object-cover">
                </a>
                ...
                <div class="flex-1 space-y-2">
                    ...
                    @if($comment->media_path)
                        <div class="mt-3">
                            @if(Str::endsWith($comment->media_path, ['jpg','jpeg','png','webp']))
                                <img src="{{ asset('storage/'.$comment->media_path) }}" class="max-w-full sm:max-w-md rounded-lg shadow-md border object-cover" alt="Imagem anexada">
                            ...
                            @else
                                <a href="{{ asset('storage/'.$comment->media_path) }}" target="_blank"
                                    ...
                                    <i class="ph-fill ph-file-text text-base"></i> Ver arquivo: {{ pathinfo($comment->media_path, PATHINFO_BASENAME) }}
                                </a>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="flex items-center gap-4 text-sm mt-3 pt-1">
                        ...
                        @php
                            $userLike = auth()->user()->commentReactions->where('comment_id', $comment->id)->where('type', 'like')->count() > 0;
                            $userDislike = auth()->user()->commentReactions->where('comment_id', $comment->id)->where('type', 'dislike')->count() > 0;
                        @endphp
                        ...
                        {{-- Ações --}}
                        <button wire:click="setReply({{ $comment->id }})" class="text-gray-600 hover:text-blue-600 flex items-center gap-1 font-medium transition-colors">
                            <i class="ph-fill ph-chat-circle-dots text-base"></i> Responder
                        </button>
    
                        @if($comment->user_id === auth()->id())
                            <button wire:click="editComment({{ $comment->id }})" class="text-gray-600 hover:text-yellow-600 flex items-center gap-1 font-medium transition-colors">
        ...
        @if ($openReplyFormId === $comment->id)
            <div class="mt-4 pl-12 pt-4 border-l border-gray-200 transition-all duration-300" id="reply-form-{{ $comment->id }}">
                <div class="bg-gray-50 p-4 rounded-lg border shadow-inner">
                     <div class="mb-3 text-sm text-gray-600 flex items-center justify-between">
                        <span>
                            <i class="ph-fill ph-arrow-u-up-left text-blue-800 mr-1"></i>
                            Respondendo a <span class="font-semibold text-blue-800">{{ $comment->user->name }}</span>
                        </span>
                        <button wire:click="$set('openReplyFormId', null)" class="text-xs text-gray-500 hover:text-gray-800 hover:underline flex items-center font-medium transition-colors">
                    ...
                    <div class="flex justify-end mt-2">
                         <button wire:click="addReply({{ $comment->id }})"
                            ...
                            <span wire:loading.remove wire:target="addReply({{ $comment->id }})">Enviar Resposta</span>
        ...
        @if ($comment->replies->count())
            <div class="mt-4 pl-12 border-l border-gray-200 space-y-4" x-data="{ repliesHidden: true }" x-persist="{{ $comment->id }}-replies">
                    ...
                    @foreach($comment->replies as $reply)
                        ...
                        <div class="flex items-start gap-3" id="comment-{{ $reply->id }}">
                            <a href="{{ route($replyProfileRoute, $reply->user->id) }}">
                                <img src="{{ $reply->user->user_icon_url }}" alt="{{ $reply->user->name }}" class="w-8 h-8 rounded-full shadow-sm object-cover">
                            </a>
                            <div class="flex-1 space-y-1">
                                ...
                                @if (!$isReplyEditing)
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <a href="{{ route($replyProfileRoute, $reply->user->id) }}" class="font-bold text-gray-800 hover:underline text-sm">
                                            {{ $reply->user->name }}
                                        </a>
                                    ...
                                    <div class="flex items-center gap-3 text-xs mt-2 pt-1">
                                        {{-- Reações --}}
                                        @php
                                            $replyLike = auth()->user()->commentReactions->where('comment_id', $reply->id)->where('type', 'like')->count() > 0;
                                            $replyDislike = auth()->user()->commentReactions->where('comment_id', $reply->id)->where('type', 'dislike')->count() > 0;
                                        @endphp
                                        ...
                                        @if($reply->user_id === auth()->id())
                                            <button wire:click="editComment({{ $reply->id }})" class="text-gray-600 hover:text-yellow-600 flex items-center gap-1 font-medium transition-colors">
    ...
    >
    {{-- Overlay --}}
    <div x-show="isModalOpen" x-transition.opacity @click="closeModal()" class="fixed inset-0 bg-gray-900 bg-opacity-75"></div>
        ...
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full z-10 mx-auto transform transition-all">
            <div class="text-center">
                ...
                <p class="text-sm text-gray-600">
                    Tem certeza que deseja excluir este comentário? Esta ação é irreversível e excluirá também todas as suas respostas.
                </p>
            </div>
            ...
            <div class="mt-6 flex justify-center gap-4">
                ...
                <button wire:click="deleteComment" 
                    @click="closeModal()" {{-- Fecha o modal imediatamente para melhor UX --}}

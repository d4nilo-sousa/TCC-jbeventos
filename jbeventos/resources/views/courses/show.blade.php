                ...
                <div class="mt-4 pt-10">
                    ...
                    <div x-data="{ isEditing: false, description: '{{ addslashes($course->course_description) }}' }">
                        <h3 class="text-base font-semibold text-stone-800 mt-4 mb-2 flex items-center gap-2">
                            Descrição do Curso
                            @if(auth()->user()->user_type === 'admin')
                                <button x-show="!isEditing" @click="isEditing = true" class="text-gray-400 hover:text-blue-500 transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                </button>
                        ...
                        @if(auth()->user()->user_type === 'admin')
                            <form x-show="isEditing" action="{{ route('courses.updateDescription', $course->id) }}" method="POST" @submit.prevent="$el.submit()">
                                @csrf
                                @method('PUT')
                                <textarea x-model="description" name="course_description" rows="5" class="w-full p-2 border rounded-md focus:ring-blue-500 focus:border-blue-500 text-sm"></textarea>
                                <div class="mt-2 flex gap-2 justify-end">
                                    <button type="button" @click="isEditing = false; description = '{{ addslashes($course->course_description) }}'" class="px-3 py-1 text-sm text-gray-700 border border-gray-300 rounded-full hover:bg-gray-100 transition">Cancelar</button>
                                    <button type="submit" class="px-3 py-1 text-sm text-white bg-blue-600 rounded-full hover:bg-blue-700 transition">Salvar</button>
                                </div>
                            </form>
                        @endif
                    </div>
                    ...
                    @if(auth()->user()->user_type === 'admin')
                        <div class="mt-6 border-t border-gray-200 pt-4 flex justify-end gap-4">
                            {{-- Botão Editar Curso --}}
                            <a href="{{ route('courses.edit', $course->id) }}" class="flex items-center gap-1 px-3 py-1 text-sm text-blue-600 hover:text-blue-700 border border-blue-300 rounded-full shadow-sm transition-colors duration-200">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                </svg>
                                Editar Curso
                            ...
                            {{-- Botão Excluir Curso (com confirmação) --}}
                            <button type="button" onclick="if(confirm('Tem certeza que deseja excluir este curso?')) { document.getElementById('deleteCourseForm').submit() }" class="flex items-center gap-1 px-3 py-1 text-sm text-red-600 hover:text-red-700 border border-red-300 rounded-full shadow-sm transition-colors duration-200">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <form id="deleteCourseForm" action="{{ route('courses.destroy', $course->id) }}" method="POST" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>

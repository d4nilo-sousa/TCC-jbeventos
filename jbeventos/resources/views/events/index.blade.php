        ...
        <div class="w-full max-w-[100rem] mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-2xl p-4 sm:p-6 lg:p-9 mx-auto min-h-[70vh]">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 md:gap-0">
                        ...
                        <form action="{{ route('events.index') }}" method="GET" class="flex items-center w-full">
                            <div class="relative w-full">
                                <input type="text" name="search" value="{{ request('search') }}"
                                    ...
                                    class="w-full rounded-full border-2 border-gray-300 bg-white py-2 pl-5 pr-12 text-gray-700 focus:border-blue-500 focus:ring-blue-500 transition-colors">
                    ...
                    {{-- Barra de pesquisa --}}
                    <form action="{{ route('events.index') }}" method="GET" class="items-center w-full sm:w-auto">
                        {{-- Adicionei mt-6 para espaçamento --}}
                        <div class="flex items-center bg-white rounded-full overflow-hidden border-2 w-full sm:w-auto">
                    ...
                    <div class="relative inline-block text-left w-full sm:w-auto">
                        ...
                        <div id="filterMenu"
                            class="absolute right-0 z-20 mt-2 w-72 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none hidden">
                                        ...
                                        <div class="mb-10">
                                            <div id="eventsList" data-url="{{ route('events.index') }}"
                                                ...
                                                @forelse ($events as $event)
                                                    @include('partials.events.event-card', [
                                                        'event' => $event,
                                                    ])
                                                @empty
                                                    <div id="noEventsMessage"
                                                        class="col-span-full flex flex-col items-center justify-center gap-5 p-10">
                                                        <img src="{{ asset('imgs/notFound.png') }}" class="w-[19%]"
                                                            alt="not-found">
                                                        <p class="text-gray-500 text-center">Nenhum evento
                                                            encontrado...</p>
                                                    </div>
            ...
            <div id="events-container" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @forelse ($events as $event)
                    <a href="{{ route('events.show', $event->id) }}"
                        ...
                        {{-- Event Card --}}
                        <div
                            class="relative bg-white border border-gray-200 rounded-xl shadow-md p-4 flex flex-col h-full">
                            {{-- Image --}}
                            <div class="relative w-full h-48 rounded-md overflow-hidden mb-4">
                                ...
                                @if ($loggedCoordinator && $loggedCoordinator->id === $event->eventCoordinator->user_account_id)
                                    @if ($event->is_visible)
                                        <span
                                            class="absolute top-2 left-2 text-xs font-semibold px-2 py-1 rounded-full shadow bg-green-500 text-white">
                                            Visível
                                        </span>
                                    @else
                                        <span
                                            class="absolute top-2 left-2 text-xs font-semibold px-2 py-1 rounded-full shadow bg-red-500 text-white">
                                            Oculto
                                        </span>
                                    @endif
                                @endif
                ...
                @empty
                    ...
                    <div class="col-span-full flex flex-col items-center justify-center p-12">
                        <img src="{{ asset('imgs/placeholder.png') }}" alt="Nenhum evento encontrado"

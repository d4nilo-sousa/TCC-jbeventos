<x-app-layout>
    <div class="relative bg-white shadow rounded-lg overflow-hidden">
        ...
        <div class="relative h-48 bg-gray-200">
            ...
            @if(auth()->id() === $user->id)
                <form method="POST" action="{{ route('profile.updateBanner') }}" enctype="multipart/form-data"
                      class="absolute top-2 right-2">
                    @csrf
                    <input type="file" name="user_banner" id="bannerUpload" class="hidden"
            ...
            <div class="relative">
                ...
                @if(auth()->id() === $user->id)
                    <form method="POST" action="{{ route('profile.updatePhoto') }}" enctype="multipart/form-data"
                        ...
                        @csrf
                        <input type="file" name="user_icon" id="photoUpload" class="hidden"
                               onchange="this.form.submit()">
                        <button type="button"
        ...
        {{-- Biografia --}}
        <div class="px-6 py-4"
             x-data="{ editing: false, bio: @js(old('bio', $user->bio)), original: @js($user->bio) }">
